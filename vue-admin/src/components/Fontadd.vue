...
<script>
import { addrepository, getrepository } from '../api/api';
...
export default {
  ...
  data: function(){
    return {
      setup_content: '#编写shell，一条命令占一行，以分号结尾',
      teardown_content: '#编写shell，一条命令占一行，以分号结尾',
      form: {
        name: '',           // 环境名称
        git: '',         // 仓库地址
        server: '',     // 简要描述
  ...
  methods: {
    ...
    async saveBuiltin() {
      ...
      this.setup_content = ''
      this.teardown_content = ''
      let build = {setup:'#编写shell，一条命令占一行，以分号结尾'+'/n'+ this.setup_content, teardown: '#编写shell，一条命令占一行，以分号结尾'+'/n'+ this.teardown_content}
      this.form.build = JSON.stringify(build)
      let response = await getrepository({page:1, size:100})
      let lists = response.data['results']
      for (var i=0; i<lists.length; i++){
        if (this.form.name === lists[i]['name']){
