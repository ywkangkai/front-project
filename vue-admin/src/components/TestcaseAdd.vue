  ...
  <div>
    ...
    <div class="container">
      ...
      <el-row>
        ...
        <el-col :span="2">
          <el-button @click="resetFor">取消</el-button>
        </el-col>
      </el-row>
    </div>
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
      ...
      // if (this.apiMsgData.url.startsWith !== 'https' || this.apiMsgData.url.startsWith !== 'http'){
    ...
    getInterfacesByProjectID(pro_id) {
      get_interfaces_by_project_id({id: pro_id})
          .then((response) => {
    ...
    unchangeResult(){
      let unseletct_testcase = this.unselected
      let len = unseletct_testcase.length;
      let text = "[";
      for (let i = 0; i < len; i++) {
        if (i === len - 1) {
          text += unseletct_testcase[i].id + "]";
        } else {
          text += unseletct_testcase[i].id + ", ";
        }
      }
    ...
    formatData() {
      // 格式化json字符串
      try {
        this.apiMsgData.jsonVariable = JSON.parse(this.apiMsgData.jsonVariable);   //将json字符串转化为JavaScript对象
        this.apiMsgData.jsonVariable = JSON.stringify(this.apiMsgData.jsonVariable, null, 4);   //将JavaScript对象转化为json字符串
      }
      catch (err) {
        this.$message({
          showClose: true,
