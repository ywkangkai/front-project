                                    ...
                                    <div class="drag-box-item">
                                        ...
                                        <draggable v-model="unselected" :options="dragOptions" @change="unchangeResult()">
                                            <transition-group tag="div" class="item-ul">
                                                <div v-for="item in unselected" class="drag-list" :key="item.id" :value="item.id">
                                                    {{ item.name }}
                                                </div>
                                            </transition-group>
                                        </draggable>
                                ...
                                <el-col :span="12">
                                    <div class="drag-box-item">
                                        ...
                                        <draggable v-model="selected" :options="dragOptions" @change="changeResult()">
                                            <transition-group tag="div" class="item-ul">
                                                <div v-for="item in selected" class="drag-list" :key="item.id" :value="item.id">
                                                    {{ item.name }}
                                                </div>
                                            </transition-group>
                                        </draggable>
...
<script>
    ...
    import {get_projects_names, interfaces_by_project_id, configures_by_interface_id, testcases_by_interface_id, get_detail_testcase, update_testcase} from '../api/api';
    ...
    export default {
        ...
        methods: {
            ...
            getInterfacesByProjectID(pro_id) {
                interfaces_by_project_id({id:pro_id})
                    .then((response) => {
            ...
            getConfTestcaseByInterfaceID(interface_id) {
                configures_by_interface_id({id:interface_id})
                    .then((response) => {
                        this.configures = response.data.data;
                    })
                    .catch(error => {
                        that.$message.error('服务器错误');
                    });

                testcases_by_interface_id({id:interface_id})
                    .then((response) => {
                      let get_unselected = response.data.data
                      ...
                      get_unselected.forEach(x=>{
                        ...
                        //this.unselected = response.data.data;
                    })
                    .catch(error => {
                        that.$message.error('服务器错误');
                    });
          ...
          unchangeResult(){
            let len = this.unselected.length
            let text = "[";
            for (let i = 0; i < len; i++) {
              if (i === len - 1) {
            ...
            console.log(this.unselected_testcase_id)
          },
            delTableRow(type, i) {
                if (type === 'variable') {
            ...
            getTestcaseDetail(){
                get_detail_testcase(this.current_testcase_id)
                    .then(response => {
                        ...
                        this.selected = response.data.selected_testcase_id;
                        this.unselected_testcase_id = response.data.unselected_testcase_id
